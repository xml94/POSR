...
from config import ivadlrose_train_root, ivadlrose_val_root
...
def splite_train_val(root):
    ...
    class_name = ['Rose_D04', 'Rose_H', 'Rose_P01',
                  'Rose_P02', 'Rose_R01', 'Rose_R02']
...
class IvadlRose(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform):
        super(IvadlRose, self).__init__(root, transform)
...
def get_ivadl_rose_datasets(train_transform, test_transform, train_classes=range(4),
                                open_set_classes=range(4, 6), balance_open_set_eval=False, split_train_val=True,
                                seed=0):
    np.random.seed(seed)

    # Init train dataset and subsample training classes
    train_dataset_whole = IvadlRose(root=ivadlrose_train_root, transform=train_transform)
    ...
    test_dataset_known = IvadlRose(root=ivadlrose_val_root, transform=test_transform)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = IvadlRose(root=ivadlrose_val_root, transform=test_transform)
...
if __name__ == '__main__':
    # splite_train_val('./datasets/IVADL_rose/raw')
    # move_file('./datasets/IVADL_rose/raw')
    x = get_ivadl_tomato_datasets(None, None, balance_open_set_eval=False, split_train_val=False)
