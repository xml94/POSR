...
from config import cottonweed_train_root, cottonweed_val_root
...
def splite_train_val(root):
    """
    splite the cotton dataset into train and val dataset
    ...
    class_name = ['Carpetweeds', 'Crabgrass', 'Eclipta', 'Goosegrass', 'Morningglory', 'Nutsedge', 'PalmerAmaranth',
                  'Prickly Sida', 'Purslane', 'Ragweed', 'Sicklepod', 'SpottedSpurge', 'SpurredAnoda', 'Swinecress',
                  'Waterhemp']
...
class CottonWeed(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform):
        super(CottonWeed, self).__init__(root, transform)
...
def get_cotton_weed_datasets(train_transform, test_transform, train_classes=range(10),
                                open_set_classes=range(10, 15), balance_open_set_eval=False, split_train_val=True,
                                seed=0):
    np.random.seed(seed)

    # Init train dataset and subsample training classes
    train_dataset_whole = CottonWeed(root=cottonweed_train_root, transform=train_transform)
    ...
    test_dataset_known = CottonWeed(root=cottonweed_val_root, transform=test_transform)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = CottonWeed(root=cottonweed_val_root, transform=test_transform)
...
if __name__ == '__main__':
    x = get_cotton_weed_datasets(None, None, balance_open_set_eval=False, split_train_val=False)
