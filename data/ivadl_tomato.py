...
from config import ivadltomato_train_root, ivadltomato_val_root
...
def splite_train_val(root):
    ...
    class_name = ['Tomato_D01_ulcer', 'Tomato_D04_leaf_fungus', 'Tomato_D05_septoria_leaf_spot',
...
class IvadlTomato(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform):
        super(IvadlTomato, self).__init__(root, transform)
...
def get_ivadl_tomato_datasets(train_transform, test_transform, train_classes=range(6),
    ...
    train_dataset_whole = IvadlTomato(root=ivadltomato_train_root, transform=train_transform)
    ...
    test_dataset_known = IvadlTomato(root=ivadltomato_val_root, transform=test_transform)
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = IvadlTomato(root=ivadltomato_val_root, transform=test_transform)
...
if __name__ == '__main__':
    # splite_train_val('./datasets/IVADL_tomato/raw')
    # move_file('./datasets/IVADL_tomato/raw')
