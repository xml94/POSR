...
from methods.ARPL.loss.LabelSmoothing import smooth_cross_entropy_loss, smooth_one_hot
from methods.ARPL.loss.AdMSLoss import AdMSoftmaxLoss
...
class ARPLoss(nn.CrossEntropyLoss):
    ...
    def __init__(self, **options):
        ...
        self.dim = -1
        self.admloss = options['admloss']
        if self.admloss:
            self.adms_loss = AdMSoftmaxLoss(s=10.0, m=0.5)
    ...
    def forward(self, x, y, labels=None):
        ...
        if not self.label_smoothing:
            if self.admloss:
                loss = self.adms_loss(logits / self.temp, labels)
            else:
                loss = F.cross_entropy(logits / self.temp, labels)

        else:
            loss = smooth_cross_entropy_loss(logits / self.temp, labels=labels, smoothing=self.label_smoothing,
                                             dim=-1)
            # labels = smooth_one_hot(true_labels=labels, smoothing=self.label_smoothing, classes=logits.size(self.dim))
            # loss = self.adms_loss(logits / self.temp, labels)
